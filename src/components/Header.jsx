...
import { Monitor, DollarSign, Wrench, ShoppingBag, Box, Settings, TrendingUp, Globe, User, BookOpen, Trophy } from 'lucide-react';
  ...
  return (
    <div className={`p-4 sticky top-0 z-50 shadow-xl border-b flex justify-between items-center transition-colors ${darkMode ? 'bg-slate-900 text-white border-slate-700' : 'bg-white text-slate-800 border-slate-200'}`}>
      ...
      <div className="hidden md:flex items-center justify-center gap-2 flex-1">
        ...
        <button onClick={() => setView('leaderboard')} className={`px-4 py-2 rounded-full flex items-center gap-2 text-sm font-bold transition-all ${view === 'leaderboard' ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/30' : 'text-slate-400 hover:bg-slate-800/50'}`}>
